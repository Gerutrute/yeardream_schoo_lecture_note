...
### 📘 정의
- 하나의 정점(노드)에서 시작하여 **인접한 모든 정점들을 먼저 탐색**한 후,
  그 다음 정점으로 넘어가는 방식 (레벨 순서 탐색)

### 🔁 동작 방식
- **큐(Queue)** 자료구조를 사용하여 구현
- 루트 노드(또는 임의의 시작 노드)에서 가까운 노드부터 차례대로 탐색

### 🎯 사용 목적
- **두 노드 간의 최단 경로**를 찾고자 할 때 유용
- 깊이보다 너비를 우선시하며, **입력 조건에서 최단 거리/경로** 조건이 있는 경우에 적합

### 🧠 비유
- 물결이 퍼지듯이 인접한 노드부터 점차 확장되어 탐색

## ✅ BFS의 특징

### 📌 탐색 방식
- **계층적으로 탐색**하는 방식 (DFS와 달리)
- **큐(Queue)** 를 사용하여 구현

### 🔁 사이클 처리
- 사이클이 있는 그래프에서는 **무한 루프에 빠지지 않도록 방문 체크** 필요
  - 중복 방문을 방지하기 위해 `visited` 배열 사용

### 🌊 탐색 형태
- **물결처럼 번지는 형태**로 진행됨  
  → 루트(시작) 노드에서 가까운 노드들을 먼저 탐색한 뒤, 점차 멀어지는 방향으로 탐색 확장

![image](https://github.com/user-attachments/assets/8b21e53d-8482-4f67-9b90-c1b6d2952d6b)

## ⏱️ BFS의 시간 복잡도

### 📊 그래프 표현 방식에 따른 시간 복잡도
- **인접 리스트(Graph as List)**:  
  `O(V + E)`  
  → 정점 수 \( V \), 간선 수 \( E \)

- **인접 행렬(Graph as Matrix)**:  
  `O(V²)`  
  → 모든 정점 쌍에 대해 접근 가능하지만 효율성은 낮음
...
- **DFS vs BFS**
  - 시간 복잡도 자체는 동일하지만, 일반적으로 **BFS가 DFS보다 약간 더 빠르게 종료**되는 경향이 있음
  - DFS는 재귀 기반 구현이 많아 **함수 호출 오버헤드**가 있음

- **그래프 구조와 탐색 목적에 따라 다르게 선택**
  - 그래프의 형태(희소 vs 밀집), 노드 수, 목표에 따라 BFS 또는 DFS 선택
  - ex) **최단 경로 탐색 → BFS**, **모든 조합 탐색 → DFS**

---

### 🎯 결론
- **효율적인 알고리즘 선택**은 문제의 특성과 그래프 구조에 따라 달라짐

## 🔍 DFS와 BFS의 공통점 및 차이점

### ✅ 공통점
- **그래프 탐색** 알고리즘으로, 시작 노드에서 목표 노드까지 도달하거나 특정 정보를 찾는 것이 목적
- **방문 기록(visited 배열 등)을 사용하여 중복 방문 방지**
  - 이미 방문한 노드를 다시 방문하지 않도록 하여 **무한 루프를 방지**
...
|------------------|-----------------------------------------------|---------------------------------------------|
| **구현 방식**    | 주로 **재귀** 사용                            | 주로 **큐(queue)** 사용                     |
| **탐색 전략**    | 한 경로를 **최대한 깊게** 탐색 후 백트래킹     | 루트에서 **가까운 노드부터 너비로** 탐색    |
| **사용 용도**    | 트리 구조 탐색, 백트래킹 문제                 | **최단 경로 탐색**에 자주 사용              |

### ✅ 문제 유형에 따른 DFS vs BFS 추천표
| 문제 유형                       | 추천 알고리즘 | 이유                                      |
|--------------------------------|----------------|--------------------------------------------|
| 모든 경로를 구하라             | DFS            | 모든 경우의 수 탐색 필요                   |
| 최단 경로를 구하라             | BFS            | 최단 거리 보장                             |
| ***트리 구조의 깊이를 구하라      | DFS            | 깊이 우선 탐색이 유리                      |
| 선택 탐색(모든 경우의 수 탐색) | DFS            | 경로 수가 많을 때 가지치기 처리에 용이    |
| 스크롤, 조희, 순환             | DFS + 백트래킹 | 경로의 수 체크 및 조건 만족 탐색 가능     |
| 가장 빠르게 도달하는가         | BFS            | 거리 기반 최단 시간 보장                   |

### 🤖 AI 문제 유형별 DFS/BFS 비교

| AI 문제 유형                   | DFS 특징                              | BFS 특징                              |
|-------------------------------|----------------------------------------|----------------------------------------|
| 퍼즐, 슬라이딩 타일           | 가능한 해를 빠르게 찾음               | **최단 횟수 해를 보장**                |
| 미로 탐색, 로봇 경로 계획     | 모든 가능성 확인                      | **최단 거리 탐색**                     |
| 게임 맵 이동 (예: 체스 등)    | 경로/깊이 탐색 구조                   | ✅ **최소 이동 횟수 탐색**             |
| 지식 추론, 규칙 전개          | 전방 탐색 구조 (깊이 중심)            | 역방향 탐색 구조 (넓이 중심)           |
| 웹 그래프 전체 탐색           | 트리 구조로 깊게 타기                | 페이지/레벨/링크 순서대로 방문         |

🧠 정리 키워드
DFS → 깊이, 경우의 수, 백트래킹
BFS → 거리, 단계, 최단 경로 탐색

![image](https://github.com/user-attachments/assets/94a29f2e-c910-4231-95e9-fe79a8dfd98e)
